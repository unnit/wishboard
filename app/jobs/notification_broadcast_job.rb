class NotificationBroadcastJob < ApplicationJob
  ...
  def perform(user)
    ...
    ActionCable.server.broadcast(
      ...
      {
        user_id: user.id,
        notification_count: @unchecked_notifications.size,
        live_notifications: render_live_notifications(@unchecked_notifications.first),
        live_id: @unchecked_notifications.first.id,
        live_class: @unchecked_notifications.first.class.name
      }
    )
  end
