class TransactionsController < ApplicationController
  ...
  before_action :check_product_owner, only: [:accept, :deny, :delete_non_coco]
  before_action :set_product, only: [:new, :create, :non_coco]
  before_action :check_session_value_and_past_dates_and_operator_type, only: [:new, :create]
  before_action :check_product_availability, only: [:new, :create]
  before_action :product_availability_for_accepted, only: [:checkout]
  before_action :basic_checks_before_checkout, only: [:checkout]
